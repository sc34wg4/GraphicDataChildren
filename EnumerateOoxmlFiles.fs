...
let allOoxmlFilesOnThisComputer1 () =
...
let rec enumerateFiles (dirInfo: DirectoryInfo) = 
    let docx =
        try dirInfo.EnumerateFiles("*.docx")
        with | e -> List.toSeq []
    let pptx =
    ...
    let subs =
        try
            let subdirs = dirInfo.EnumerateDirectories()
            seq { for subdir in subdirs do yield! enumerateFiles subdir }
        with | e -> List.toSeq []
    Seq.append subs (Seq.append docx (Seq.append pptx xlsx))
...
let allOoxmlFilesOnThisComputer () =
    ...
    [ for drive in drives do
            if drive.IsReady  then
                System.Console.WriteLine(drive.Name)
                let ooxmlFiles = enumerateFiles(drive.RootDirectory)
                yield! ooxmlFiles ]